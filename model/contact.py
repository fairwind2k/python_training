...
class Contact:
    ...
    def __repr__(self):
        return "%s:%s:%s:%s;%s:%s:%s:%s;%s:%s:%s:%s;%s" % (self.id, self.firstname, self.lastname , self.address, self.homephone,
                      self.mobile, self.workphone, self.fax, self.email,
                      self.email2, self.email3, self.homepage, self.secondaryphone)
